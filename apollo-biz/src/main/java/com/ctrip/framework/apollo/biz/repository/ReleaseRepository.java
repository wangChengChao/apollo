...
public interface ReleaseRepository extends PagingAndSortingRepository<Release, Long> {
  ...
  List<Release> findByAppIdAndClusterNameAndNamespaceNameOrderByIdDesc(
  ...
  @Query(
      nativeQuery = true,
      value =
          "update \"Release\" set \"IsDeleted\"=1,\"DataChange_LastModifiedBy\" = ?4 where \"AppId\"=?1 and \"ClusterName\"=?2 and \"NamespaceName\" = ?3")
  int batchDelete(String appId, String clusterName, String namespaceName, String operator);

  // For release history conversion program, need to delete after conversion it done
  List<Release> findByAppIdAndClusterNameAndNamespaceNameOrderByIdAsc(
