...
function directive($window, toastr, AppUtil, EventManager, PermissionService,
    NamespaceLockService,
    UserService, CommitService, ReleaseService, InstanceService,
    NamespaceBranchService, ConfigService) {
  return {
    restrict: 'E',
    ...
    link: function (scope) {
      ...
      var subscriberId = EventManager.subscribe(
          EventManager.EventType.UPDATE_GRAY_RELEASE_RULES,
          function (context) {
      ...
      function preInit(namespace) {
        ...
        namespace.isLinkedNamespace =
            namespace.isPublic ? namespace.parentAppId
                != namespace.baseInfo.appId : false;
        //namespace view name hide suffix
        namespace.viewName = namespace.baseInfo.namespaceName.replace(".xml",
            "").replace(
      ...
      function initNamespace(namespace, viewType) {
        ...
        scope.showNamespaceBody = namespace.showNamespaceBody ? true
            : scope.showBody;
        namespace.viewItems = namespace.items;
        namespace.isPropertiesFormat = namespace.format == 'properties';
        namespace.isSyntaxCheckable = namespace.format == 'yaml'
            || namespace.format == 'yml';
        ...
        function initNamespaceBranch(namespace) {
          ...
          .then(function (result) {
            ...
            namespace.branch.isPropertiesFormat = namespace.format
                == 'properties';
        ...
        function initLinkedNamespace(namespace) {
          ...
          ConfigService.load_public_namespace_for_associated_namespace(
              scope.env, scope.appId, scope.cluster,
              namespace.baseInfo.namespaceName)
          .then(function (result) {
        ...
        function initUserOperateBranchScene(namespace) {
          var operateBranchStorage = JSON.parse(
              localStorage.getItem(operate_branch_storage_key));
          var namespaceId = [scope.appId, scope.env, scope.cluster,
            namespace.baseInfo.namespaceName].join(
              "+");
          ...
          localStorage.setItem(operate_branch_storage_key,
              JSON.stringify(operateBranchStorage));
      ...
      function switchBranch(branchName, forceShowBody) {
        ...
        var namespaceId = [scope.appId, scope.env, scope.cluster,
          scope.namespace.baseInfo.namespaceName].join(
            "+");
      ...
      scope.aceConfig = {
        ...
        theme: 'eclipse',
        mode: scope.namespace.format === 'yml' ? 'yaml'
            : (scope.namespace.format === 'txt' ? undefined
