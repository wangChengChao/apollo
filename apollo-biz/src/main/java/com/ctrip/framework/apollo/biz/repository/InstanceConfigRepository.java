...
public interface InstanceConfigRepository extends PagingAndSortingRepository<InstanceConfig, Long> {
  ...
  InstanceConfig findByInstanceIdAndConfigAppIdAndConfigNamespaceName(
      long instanceId, String configAppId, String configNamespaceName);

  Page<InstanceConfig> findByReleaseKeyAndDataChangeLastModifiedTimeAfter(
      String releaseKey, Date validDate, Pageable pageable);
  ...
  @Query(
      nativeQuery = true,
      value =
          "delete from \"InstanceConfig\"  where \"ConfigAppId\"=?1 and \"ConfigClusterName\"=?2 and \"ConfigNamespaceName\" = ?3")
  int batchDelete(String appId, String clusterName, String namespaceName);

  @Query(
      value =
          "select b.Id from \"InstanceConfig\" a inner join \"Instance\" b on b.\"Id\" ="
      ...
      countQuery =
          "select count(1) from \"InstanceConfig\" a inner join \"Instance\" b on b.\"Id\" ="
              + " a.\"InstanceId\" where a.\"ConfigAppId\" = :configAppId and a.\"ConfigClusterName\" = "
              + ":clusterName and a.\"ConfigNamespaceName\" = :namespaceName and a.\"DataChange_LastTime\" "
              + "> :validDate and b.\"AppId\" = :instanceAppId",
      nativeQuery = true)
  Page<Object> findInstanceIdsByNamespaceAndInstanceAppId(
      @Param("instanceAppId") String instanceAppId,
      @Param("configAppId") String configAppId,
